...
export default function InvitationList() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["invitation", { gql: queryDocument(searchTerm, currentPage) }],
  ...
  return (
    ...
    >
      <JumboCard contentWrapper>
        ...
        <Typography variant={"h3"}>{t("Invitations")}</Typography>
        <BListWrapper>
          <header>
            <BTHeaderWrapper>
              <Stack>
    ...
    <>
      <BList>
        <Stack>
          <BTitle variant="h5">
            {" "}
            {item.linkedUser ? item.linkedUser.email : item.email}
          </BTitle>
        </Stack>
        <Stack>
          <ButtonGroup>
