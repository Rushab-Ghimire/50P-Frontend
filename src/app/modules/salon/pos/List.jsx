...
import { getDocument as floorplanQD } from "../floorplan/Documents";
...
export default function PosList() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: [
      "pos",
      { gql: queryDocument(params.id, searchTerm, currentPage) },
    ],
  ...
  const {
    ...
    queryKey: ["pos", { gql: floorplanQD(params.id) }],
    queryFn: ({ signal, queryKey }) =>
      gqlQuery({ signal, path: "/graphql", inData: queryKey[1] }),
  });
    ...
    >
      <JumboCard contentWrapper>
        ...
        <Typography variant={"h3"}>
          ...
          {t(`Point of Sales for Floorplan : ${!dataFL ? "-" : dataFL.title}`)}
        </Typography>
        <BListWrapper>
          <header>
            <Box
            ...
            >
              ...
              <Stack>
                <Link to={`/salon/floorplan/${params.id}/pos/new`}>
                  <Button
                    variant={"contained"}
                    size={"small"}
                    color={"primary"}
        ...
        <Stack>
          <ButtonGroup>
            <Button
              ...
              onClick={() => {
                navigate(`/salon/floorplan/${params.id}/pos/${item.id}`);
              }}
            >
              Edit
            </Button>
