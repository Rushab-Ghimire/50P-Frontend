...
import CustomerProfileWidget from "./components/CustomerProfileWidget";
...
import CustomerVoucher from "./components/CustomerVoucher";
import { CWTgBtn } from "@app/_styles/CustomerWidgets";
import { getCustomerDataset } from "@app/modules/salon/customers/Documents";
...
var data = {
  ...
  notes: [
    ...
    {
      from: "Beautician 3",
      note: "Easy to work with",
    },
  ],
};

const CustomerWidget = ({ customerId }) => {
  ...
  const {
    ...
    queryKey: ["customerSummary", { gql: getCustomerDataset(customerId) }],
    ...
    <>
      <Grid container spacing={4} sx={{ padding: "25px 40px 0 40px" }}>
        <Grid item xs={12} md={6}>
          <CustomerProfileWidget profile={data?.profile} />
          <Box>
            <ToggleButtonGroup
              color="primary"
              value={highlight}
              ...
              sx={{ width: "100%" }}
            >
              <CWTgBtn value="details" onClick={showDetails}>
                Details
              </CWTgBtn>
              <CWTgBtn value="appointments" onClick={showAppointments}>
                Appointments
              </CWTgBtn>
              <CWTgBtn value="vouchers" onClick={showVouchers}>
                Vouchers
              </CWTgBtn>
              <CWTgBtn value="invoices" onClick={showInvoices}>
                Invoices
              </CWTgBtn>
            </ToggleButtonGroup>
          </Box>
        ...
        <Grid item xs={12} md={6}>
          <Grid item xs={12}>
            {invoice && <CustomerInvoice invoices={data?.invoices} />}
            {voucher && <CustomerVoucher vouchers={data?.vouchers} />}
            {appointment && (
              <CustomerAppointment appointments={data?.appointments} />
            )}
            ...
            {widgets && (
              <React.Fragment>
                ...
                <ObjectCountOrders
                  ...
                  color="warning.main"
                />
                <NoteWidget title="Client Notes" notes={data?.notes} />
              </React.Fragment>
            )}
          </Grid>
        </Grid>
