...
import StaffWidget from "../modals/StaffWidget";
...
export default function StaffList() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["staff", { gql: queryDocument(searchTerm, currentPage) }],
  ...
  return (
    ...
    >
      <JumboCard contentWrapper>
        ...
        <Typography variant={"h3"}>{t("Staffs")}</Typography>
        <BListWrapper>
          <header>
            <BTHeaderWrapper>
              <Stack>
  ...
  return (
    <>
      <BList>
        <Stack sx={{ flex: 1 }}>
          <BTitle variant="h5">
            {item.linkedUser.firstName} {item.linkedUser.lastName}
          </BTitle>
        </Stack>
        <Stack sx={{ flex: 1 }}>
          <Span variant="h6">{item.linkedUser.email}</Span>
