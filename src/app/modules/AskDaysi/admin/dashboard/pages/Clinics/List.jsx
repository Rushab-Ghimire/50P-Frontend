...
export default function ClinicsList() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["clinics", { gql: queryDocument(searchTerm, currentPage) }],
  ...
  return (
    ...
    >
      <JumboCard contentWrapper>
        ...
        <Typography variant={"h3"}>{t("My Clinics")}</Typography>
        <BListWrapper>
          <header>
            <BTHeaderWrapper>
              <Stack>
        ...
        <Stack>
          <ButtonGroup>
            <Button
              ...
              onClick={() => {
                navigate(`/askdaysi/clinics/${item.id}`);
              }}
            >
              Edit
            </Button>
