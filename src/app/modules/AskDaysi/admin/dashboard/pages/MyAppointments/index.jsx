...
export default function MyAppointments() {
  ...
  if (data && data.rows.length > 0) {
    ...
    if (searchParams.get("bid") !== -1) {
      setTimeout(() => {
        jQuery("#edit-myapps-" + searchParams.get("bid")).click();
        console.log("click", searchParams.get("bid"));
    ...
    <>
      <BList>
        <Stack sx={{ width: { xs: "100%", sm: "auto", md: "33%" } }}>
          <BTitle variant="h5">{item.provider.name}</BTitle>
        </Stack>
        <Stack>
          <BTitle variant="h5" sx={{ lineHeight: "24px" }}>
            {getFormattedDate(item.bookingDateTime)}
            <br />
            Status :{" "}
            {item.status == "new" && (
              <ButtonGroup>
        ...
        <Stack>
          <ButtonGroup>
            ...
            <Button
              ...
              color={"success"}
              onClick={() => {
                window.showGeneralDialog("DoctorPatientChat", item.id);
              }}
            >
              Chat
            </Button>
