...
export default function InsurancesList() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["insurances", { gql: queryDocument(searchTerm, currentPage) }],
  ...
  return (
    ...
    >
      <JumboCard contentWrapper>
        ...
        <Typography variant={"h3"}>{t("Insurance Providers")}</Typography>
        <BListWrapper>
          <header>
            <BTHeaderWrapper>
              <Stack>
              ...
              <Stack>
                <Link to={`/askdaysi/insurances/new`}>
                  <Button
                    variant={"contained"}
                    size={"small"}
                    color={"primary"}
...
export function ListItem({ item }) {
  ...
  return (
    <>
      <BList>
        <Stack>
          <BTitle variant="h5"> {item.name}</BTitle>
        </Stack>
        <Stack>
          <ButtonGroup>
            <Button
              ...
              onClick={() => {
                navigate(`/askdaysi/insurances/${item.id}`);
              }}
            >
              Edit
            </Button>
