...
export default function LabsList() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["labs", { gql: queryDocument(searchTerm, currentPage) }],
  ...
  return (
    ...
    >
      <JumboCard contentWrapper>
        ...
        <Typography variant={"h3"}>{t("Diagnostic Labs")}</Typography>
        <BListWrapper>
          <header>
            <BTHeaderWrapper>
              <Stack>
        ...
        <Stack>
          <ButtonGroup>
            <Button
              ...
              onClick={() => {
                navigate(`/askdaysi/labs/${item.id}`);
              }}
            >
              Edit
            </Button>
