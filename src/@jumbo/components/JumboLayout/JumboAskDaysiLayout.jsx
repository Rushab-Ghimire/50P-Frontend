...
import chatIcon from "/assets/images/chat-icon.png";
import AskDaysiChatWindow from "@app/modules/AskDaysi/admin/dashboard/components/ChatWindow/ChatWindow";
...
function JumboAskDaysiLayout(props) {
  const {
    rootOptions,
    sidebarOptions,
    headerOptions,
  ...
  const [open, setOpen] = useState(isProvider ? true : false);

  const checkRoleForChat = useCallback(() => {
    //console.log("appbar", getRoles(GLOBAL.userDetail.organizations));
    ...
    if (getRoles(GLOBAL.userDetail.organizations).includes("doctor")) {
      setIsProvider(true);
      setOpen(true);
    } else {
      setIsProvider(false);
    ...
    >
      ...
      >
        <Box
          ...
          className={
            isProvider && open
              ? "CmtLayout-wrapper chatOpen"
              : "CmtLayout-wrapper chatClose"
          }
        ...
        >
          ...
          <Div
            onClick={() => {
              if (
                getComputedStyle(document.getElementById("sidebar_div"))[
                  "visibility"
                ] == "visible"
              ) {
                if (
            ...
            {sidebarOptions?.style !== SIDEBAR_STYLES.CLIPPED_UNDER_HEADER && (
              <JumboLayoutHeader>
                ...
                {isProvider && (
                  ...
                  >
                    <img src={chatIcon} style={imgStyle} alt="DaysiAI Chat" />
                  </IconButton>
                )}
              </JumboLayoutHeader>
            )}
